...
void Chunk::Init(size_t block_size, uint8_t num_blocks) noexcept {
  // allocate new memory
  p_data_ = std::make_unique<uint8_t[]>(block_size * num_blocks);

  // update data end pointer. Points at past-the-end block
  p_data_end_ = p_data_.get() + (block_size * num_blocks);
...
void* Chunk::Allocate(size_t block_size) noexcept {
  ...
  uint8_t* resultPtr = p_data_.get() + (next_available_block_ * block_size);
...
void Chunk::Deallocate(void* p_block, size_t block_size) noexcept {
  ...
  *pBlockToRelease = next_available_block_;
  ...
  next_available_block_ = static_cast<uint8_t>((pBlockToRelease - p_data_.get()) / block_size);
...
bool Chunk::IsInChunk(const void* p_block) const noexcept {
  ...
  return (p_block_to_check >= p_data_.get()) && (p_block_to_check < p_data_end_);
}
